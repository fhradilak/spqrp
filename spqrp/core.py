...
from .helpers import get_sample_relations_by_cutoff, is_better_result
...
def calculate_pairwise_distances(
    ...
    distance_matrix, df_pivot = get_distances(
        df_dist, metric=metric, fractional_p=fractional_p
    )
...
def perform_distance_evaluation_on_ranked_proteins(
    df,
    top_importance_path=files("spqrp").joinpath(
        "data/ranked_classification_importance_cohort_a.csv"
    ),
    ...
    results_distances = calculate_pairwise_distances(
    ...
    distance_matrix = results_distances["distance_matrix"]
    df_dist = results_distances["df_dist"]
    sample_order = results_distances["sample_order"]
    nearest_neighbours = results_distances["nearest_neighbours"]
    ...
    if threshold_based:
        ...
        sample_patient_mapping = dict(
            df_dist[["Sample_ID", "Patient_ID"]].drop_duplicates().values
        )
        print("Real Number Proteins", len(df_dist["Protein"].unique()))
        results = get_sample_relations_by_cutoff(
            distance_matrix,
            cutoff=cutoff,
        ...
        plot_distribution_with_highlights(
            distances,
            eval_metrics["False_Negative_Distances"],
            eval_metrics["False_Positive_Distances"],
            percentiles=percentiles,
            name=name + f" {n} proteins",
        )
    else:
        belonging = []
        not_belonging = []
...
def optimize_parameters(
    ...
    optimization_strategy: Literal[
        "fp+fn", "fp", "fn", "F1", "precision", "sensitivity"
    ] = "fp+fn",
    remove_list=[],
    quiet=False,
):
    ...
    with open(log_file, "w") as log:
        ...
        for n in n_values:
            ...
            top_importance = top_importance[
                ~top_importance["Protein"].isin(remove_list)
            ]

            ### Get only overlapping proteins
            # important since we only have a sub overlapp with the proteom
            top_prot = (
                top_importance[top_importance["Protein"].isin(df["Protein"])]
            ...
            sample_patient_mapping = (
                df_dist.drop_duplicates(subset=["Sample_ID", "Patient_ID"])
                .set_index("Sample_ID")["Patient_ID"]
                .to_dict()
            )

            for fractional_p, percentile in itertools.product(
                fractional_p_values, percentile_values
            ):
                ...
                sample_order = list(df_pivot.index)
                log.write(
                    f"Testing n={n}, fractional_p={fractional_p}, percentile={percentile}\n"
                )
                ...
                belonging, not_belonging = (
                    results["belonging"],
                    results["not_belonging"],
                )
                ...
                # Log current results
                log.write(
                    f"Precision: {result['Precision']}, Sensitivity: {result['Sensitivity']}, TP:{result['TP']}, FP:{result['FP']}, TN:{result['TN']}, FN:{result['FN']}\n\n"
                )
...
def cluster_samples_iteratively(
    ...
    G, coords_2d = create_graph_based_on_reduction_method(
        method,
        n_samples,
        dist_matrix,
        sample_names,
...
def plot_distances_neighbours_with_coloring_hue(
    ...
    (
        nodes_in_tp_clusters,
        nodes_with_red_connections,
        singleton_nodes,
        isolated_nodes,
    ) = identify_clusters_singletons(
    ...
    for name in connected_samples:
        ...
        if highlight_single_samples_missing_connections and name in isolated_nodes:
            circle = plt.Circle(
                (x, y),
                0.05,
                edgecolor=error_candidate_color,
                facecolor="none",
                linewidth=2,
            )
            ax.add_patch(circle)
    ...
    transitive_results = transitive_performance(
        sample_names, drawn_pairs, sample_to_patient
    )

    # === Legends ===
    handles = []
    fp_edge_handle = mlines.Line2D(
        [],
        [],
        color=fp_color,
        linestyle=fp_linestyle,
