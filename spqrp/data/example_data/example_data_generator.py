...
for pid in range(1, n_patients + 1):
    ...
    for s in range(1, samples_per_patient + 1):
        ...
        sample_intensity = patient_baseline + np.random.normal(0, noise_level, len(proteins))
        sample_intensity = np.clip(sample_intensity, 0, 1)  # keep between 0 and 1, can be adjusted to represent e.g. log_2 transformed data
        
        for prot, intensity in zip(proteins, sample_intensity):
            data.append([sample_id, patient_id, prot, np.round(intensity, 2)])

df_generated = pd.DataFrame(data, columns=["Sample_ID", "Patient_ID", "Protein", "Intensity"])
