...
def check_required_columns(df,required_columns, type):
    ...
    print(f"Checking if columns: {required_columns} are excisting for {type}")
    if missing_columns:
        print(f"For {type} the following required columns are missing from the DataFrame: {missing_columns}. You probably have to rename the columns of the dataset to fit the requirements.")
        raise ValueError(f"For {type} the following required columns are missing from the DataFrame: {missing_columns}. You probably have to rename the columns of the dataset to fit the requirements.")
    else:
        print(f"All required columns are present for {type}.")

def check_input_data_format(df, importance_ranking):
    check_required_columns(df=df,required_columns=required_columns_df, type="protein matrix")
    check_required_columns(df=importance_ranking,required_columns=required_columns_ranking, type="importance ranking")
...
def get_distances(df_dist, metric="correlation", intensity="Intensity", kernelpca=False, fractional_p=0.5, index = SAMPLE,):
    
    df_pivot = df_dist.pivot_table(index=index, columns="Protein", values=intensity, aggfunc="mean")
    df_pivot.fillna(0, inplace=True)
    fractional = metric == ("fractional")

    if fractional:
        distance_matrix = pairwise_distances(df_pivot, metric="minkowski", p=fractional_p)
    else:   
...
def plot_distribution_of_pairwise_dist(distances, percentiles =[1,2,5,10]):
    ...
    for p in percentiles:
        plt.axvline(np.percentile(distances, p), color='#DC267F', linestyle='dashed', linewidth=2, label=f"{p}th Percentile")
    plt.legend()
    plt.show()
    
    
def plot_distribution_with_highlights(distances, fn_distances, fp_distances, percentiles=[1,2,5,10], name=""):
    plt.figure(figsize=(8, 5))
    plt.hist(distances, bins=40, alpha=0.5, color='grey', label="All Distances")
    for fn in fn_distances:
        plt.axvline(fn, color='#648FFF', linestyle='-', alpha=0.2, label="False Negatives" if 'False Negatives' not in plt.gca().get_legend_handles_labels()[1] else "")
    for fp in fp_distances:
        plt.axvline(fp, color='#FFB000', linestyle='-', alpha=0.2, label="False Positives" if 'False Positives' not in plt.gca().get_legend_handles_labels()[1] else "")

    for p in percentiles:
        plt.axvline(np.percentile(distances, p), color='#DC267F', linestyle="-", linewidth=3, label=f"{p}th Percentile")
...
def get_evaluation_metrics(belonging, not_belonging, quiet=True):
    ...
    true_positives = list(zip(belonging.loc[tp_mask, "sample1"], belonging.loc[tp_mask, "sample2"]))
    true_positive_distances = belonging.loc[tp_mask, "distance"].tolist()

    # False positives = different patients
    fp_mask = ~tp_mask
    false_positives = list(zip(belonging.loc[fp_mask, "sample1"], belonging.loc[fp_mask, "sample2"]))
    ...
    true_negatives = list(zip(not_belonging.loc[tn_mask, "sample1"], not_belonging.loc[tn_mask, "sample2"]))
    true_negative_distances = not_belonging.loc[tn_mask, "distance"].tolist()

    # False negatives = same patients
    fn_mask = ~tn_mask
    false_negatives = list(zip(not_belonging.loc[fn_mask, "sample1"], not_belonging.loc[fn_mask, "sample2"]))
    ...
    precision = true_belonging / (true_belonging + false_belonging) if (true_belonging + false_belonging) > 0 else 0
    sensitivity = true_belonging / (true_belonging + false_not_be) if (true_belonging + false_not_be) > 0 else 0
    balanced_accuracy = 0.5 * ((true_belonging / (true_belonging + false_not_be)) + (true_not_be / (true_not_be + false_belonging)))
    ...
    return dict(TP=true_belonging, FP=false_belonging, FN=false_not_be, TN=true_not_be,
                Accuracy=accuracy, Balanced_Accuracy=balanced_accuracy, Precision=precision, Sensitivity=sensitivity, F1=f1,
                False_Negative_Pairs=false_negatives, False_Negative_Distances=false_negative_distances,
                False_Positive_Pairs=false_positives, False_Positive_Distances=false_positive_distances,
                True_Negative_Pairs=true_negatives, True_Negative_Distances=true_negative_distances,
                True_Positive_Pairs=true_positives, True_Positive_Distances=true_positive_distances)
